...
import { Plus, FileUp, Clock, CheckCircle, ChevronRight, BookOpen, Globe, Settings } from "lucide-react";
...
export default function TrainingPage() {
  ...
  const [questionLimit, setQuestionLimit] = useState(0); // 0 = все вопросы
  ...
  return (
    <div data-theme={theme}>
      ...
      {/* Settings panel — количество вопросов */}
      {showSettings && (
        <div className="rounded-2xl p-4 mb-4 card-glow" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
          ...
          <div>
            ...
            <p className="text-xs mt-1" style={{ color: "var(--muted-foreground)" }}>
              {questionLimit === 0
                ? (lang === "ru" ? "Все вопросы, случайный порядок" : "Ҳама саволҳо, тартиби тасодуфӣ")
                : (lang === "ru" ? `${questionLimit} случайных вопросов` : `${questionLimit} саволи тасодуфӣ`)}
            </p>
          </div>
        </div>
      )}
...
function TestCard({ test, onStart, onEdit, onDelete, lang, tr, userId }: any) {
