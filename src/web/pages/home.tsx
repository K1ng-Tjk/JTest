...
export default function HomePage() {
  ...
  useEffect(() => {
    if (!user) return;
    api.getTests().then((res: any) => {
      const approved = (res.tests || []).filter((t: any) => t.status === "approved");
      setTotalTests(approved.length);
    });
  }, [user]);
  ...
  return (
    <div data-theme={theme}>
      ...
      {/* Один счётчик — всего тестов */}
      <div className="rounded-2xl p-5 mb-6 card-glow flex items-center justify-between"
        style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
        <div>
          <p className="text-xs mb-1" style={{ color: "var(--muted-foreground)" }}>
            {lang === "ru" ? "Всего тестов в банке" : lang === "tj" ? "Ҳамаги тестҳо" : "Total tests"}
          </p>
          <p className="text-4xl font-black" style={{ color: "var(--primary)" }}>{totalTests}</p>
        </div>
        <div className="text-5xl opacity-30">📚</div>
      </div>

      {/* Quick actions — только 2 */}
